...
def isOleContent (content):
    """
    Test if the content of a file is an OLE container (according to its header).
    return: True if OLE, False otherwise.
    """
    header = content[:len(MAGIC)]
    if header == MAGIC:
        return True
    else:
        return False
...
#--- OleFileIO ----------------------------------------------------------------
#I modified this class that parses a content rather than a filename
class OleFileIO:
    """
    ...
    def __init__(self, content = None, raise_defects=DEFECT_FATAL):
        """
    ...
    def open(self, content):
        ...
        header = content[:512] #self.fp.read(512)
        ...
        filesize = len(content)#os.path.getsize(filename)
    ...
    def getsect(self, sect):
        ...
        try:
            ...
            debug('getsect(): sect=%X, seek=%d, filesize=%d' %
                (sect, self.sectorsize*(sect+1), len(self.content)#os.path.getsize(self.fp.name)))
